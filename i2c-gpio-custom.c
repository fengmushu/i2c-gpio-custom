...
static int __init i2c_gpio_custom_add_one(unsigned int id, unsigned int *params)
{
	...
	if (!pdev) {
		printk(KERN_ERR PFX "platform_device_alloc failed: %d\n", id);
		err = -ENOMEM;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0)
		goto err_remove;
#else
	...
	if (err) {
		printk(KERN_ERR PFX "platform_device_add_data failed: %d\n", id);
		goto err_put;
	}

	err = platform_device_add(pdev);
	if (err) {
		printk(KERN_ERR PFX "platform_device_add failed: %d\n", id);
		goto err_put;
	}

	devices[nr_devices++] = pdev;

	printk(KERN_ERR PFX "platform_device_add: %d\n", id);
	return 0;
